...
public class PayPsdInputView extends androidx.appcompat.widget.AppCompatEditText {
    ...
    private void getAtt(AttributeSet attrs) {
        TypedArray typedArray = mContext.obtainStyledAttributes(attrs, me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView);
        maxCount = typedArray.getInt(me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView_maxCount, maxCount);
        circleColor = typedArray.getColor(me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView_circleColor, circleColor);
        bottomLineColor = typedArray.getColor(me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView_bottomLineColor, bottomLineColor);
        radius = typedArray.getDimensionPixelOffset(me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView_radius, radius);

        divideLineWidth = typedArray.getDimensionPixelSize(me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView_divideLineWidth, divideLineWidth);
        divideLineColor = typedArray.getColor(me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView_divideLineColor, divideLineColor);
        psdType = typedArray.getInt(me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView_psdType, psdType);
        rectAngle = typedArray.getDimensionPixelOffset(me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView_rectAngle, rectAngle);
        focusedColor = typedArray.getColor(me.jessyan.armscomponent.commonres.R.styleable.public_PayPsdInputView_focusedColor, focusedColor);
    ...
    private void initPaint() {
        ...
        textPaint = new Paint(getPaint(1, Paint.Style.FILL, circleColor));
    ...
    private void drawPsdCircle(Canvas canvas) {
        for (int i = 0; i < textLength; i++) {
            if (getInputType() == InputType.TYPE_CLASS_NUMBER) {
                //绘制文本
                String curChar = getText().toString().substring(i, i + 1);
                canvas.drawText(curChar, startX + i * 2 * startX, startY, textPaint);
            } else {
                //绘制圆点
